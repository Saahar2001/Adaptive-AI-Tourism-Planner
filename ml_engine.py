...
def fetch_ml_recommendations(city, interests, trip_month, top_k=10,
    ...
    candidates["budget_fit"] = candidates["estimated_cost"].apply(
        lambda cost: calculate_budget_score(cost, budget)
    )
    candidates["budget_difference"] = (
        candidates["estimated_cost"] - float(budget)
...
def recommend_places(city, interests, budget=None, features_df=None, top_n=10,
    ...
    if ml_df is not None and not ml_df.empty:
        ...
        result = _balance_categories(backfilled, allowed_categories, top_n, score_col="recommendation_score")
    ...
    places["budget_difference"] = (
        places["estimated_cost"] - float(budget)
        if budget is not None and pd.notna(budget)
        else 0.0
    )
    places["recommendation_score"] = (
        places["preference_match"] * 0.40
        + places["budget_fit"] * 0.20

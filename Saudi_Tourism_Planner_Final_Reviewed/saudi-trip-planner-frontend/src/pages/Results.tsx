...
export default function Results() {
  ...
  }, [activeCategory, availableCategories.join("|")]);
        ...
        {result.warnings && result.warnings.length > 0 && (
          <div className="mt-4 space-y-1">
            {result.warnings.map((warning, index) => (
              <p key={index} className="font-body text-xs text-ink-700/65">ℹ️ {warning}</p>
            ))}
          </div>
        )}
              ...
              {day?.stops.map((stop, i) => (
                <li key={i} className="border border-ink-900/10 rounded-xl p-4 bg-white/60 flex items-start justify-between gap-4">
                  ...
                  {typeof stop.estimated_cost === "number" && (
                    <span className="text-sm text-palm-700 font-medium whitespace-nowrap">Est. {stop.estimated_cost} SAR</span>
                  )}
                </li>
              )) ?? (
